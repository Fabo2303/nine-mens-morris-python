...
from game.evaluate.evaluate_phases import evaluate_drop_phase, find_complete_mills


def get_best_drop_move_hard(board, player, opponent):
    best_move = None
    best_eval = -math.inf
    for move in generate_drop_moves(board):
        ...
        eval = minimax_drop_phase(new_board, 3, False, player_copy, opponent_copy)
        if eval > best_eval:
            best_eval = eval
            best_move = move
    return best_move
